...
const contractName = "level";

const defaultNftAssetContract = "kids-nft";
const defaultPaymentAssetContract = "level-token";
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const order: Order = { tokenId, expiry: 10, price: 10 };
    const bogusNftAssetContract = `${deployer.address}.other-nft`;
    const block = chain.mineBlock([
      whitelistAssetTx(nftAssetContract, true, deployer),
      listOrderTx(nftAssetContract, maker, order),
      Tx.contractCall(
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const bogusPaymentAssetContract = `${deployer.address}.other-token`;
    const order: Order = { tokenId, expiry: 10, price, paymentAssetContract };
    const block = chain.mineBlock([
      whitelistAssetTx(nftAssetContract, true, deployer),
      whitelistAssetTx(paymentAssetContract, true, deployer),
